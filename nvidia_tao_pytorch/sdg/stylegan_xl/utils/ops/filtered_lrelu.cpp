...
static std::tuple<torch::Tensor, torch::Tensor, int64_t> filtered_lrelu(
    torch::Tensor x, torch::Tensor fu, torch::Tensor fd, torch::Tensor b, torch::Tensor si,
    int64_t up, int64_t down, int64_t px0, int64_t px1, int64_t py0, int64_t py1, int64_t sx, int64_t sy, double gain, double slope, double clamp, bool flip_filters, bool writeSigns)
{
    // Set CUDA device.
...
static torch::Tensor filtered_lrelu_act(torch::Tensor x, torch::Tensor si, int64_t sx, int64_t sy, double gain, double slope, double clamp, bool writeSigns)
{
    // Set CUDA device.
    TORCH_CHECK(x.is_cuda(), "x must reside on CUDA device");
    const at::cuda::OptionalCUDAGuard device_guard(device_of(x));
...
//------------------------------------------------------------------------
static auto registry_filtered_lrelu = torch::RegisterOperators("nvidia::filtered_lrelu", &filtered_lrelu);
static auto registry_filtered_lrelu_act_ = torch::RegisterOperators("nvidia::filtered_lrelu_act_", &filtered_lrelu_act);
