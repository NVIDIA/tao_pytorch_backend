...
def window_reverse(windows, window_size, H, W, B):
    """
    Args:
        windows (torch.Tensor): local window features
            (num_windows * B, window_size, window_size, C)
        window_size (int): Window size
        H (int): Height of image
        W (int): Width of image
        B (int): Batch size of image
    Returns:
        x: (B, H, W, C)
    """
...
class PosEmbMLPSwinv2D(nn.Module):
    """
    2D SwinV2-like Position Embedding
    """
    ...
    def __init__(self, window_size, pretrained_window_size, num_heads, seq_length, ct_correct=False, no_log=False):
        ...
        Args:
            ...
            pretrained_window_size: pretrained window size.
            num_heads: number of attention head.
            seq_length: sequence length.
            ct_correct: carrier token correction flag.
            no_log: not apply log flag.
        """
        super().__init__()
...
class PosEmbMLPSwinv1D(nn.Module):
    """
    1D SwinV2-like Position Embedding
    """
...
@BACKBONE_REGISTRY.register()
def faster_vit_4_21k_224(**kwargs):
...
@BACKBONE_REGISTRY.register()
def faster_vit_4_21k_384(**kwargs):
...
@BACKBONE_REGISTRY.register()
def faster_vit_4_21k_512(**kwargs):
...
@BACKBONE_REGISTRY.register()
def faster_vit_4_21k_768(**kwargs):
