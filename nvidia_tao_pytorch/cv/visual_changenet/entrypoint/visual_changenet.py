...
"""'Entry point script running subtasks related to visual_changenet."""
...
import nvidia_tao_pytorch.cv.visual_changenet.scripts as scripts
...
def launch(parser, subtasks, network=None):
    ...
    if args.gpus > 1:
        if args.subtask == "export":
            ...
            script_args += " num_gpus=" + str(args.gpus)
    ...
    # Add encryption key.
    if args.subtask in ["train", "evaluate", "inference", "export", "preprocess"]:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        "visual_changenet", add_help=True, description="TAO Toolkit"
    )
