...
"""BEVFusion bev pooling ops functions"""
...
class QuickCumsum(torch.autograd.Function):
    ...
    def forward(ctx, x, geom_feats, ranks):
        """ Forward function for QuickCumsum Class """
        x = x.cumsum(0)
    ...
    def backward(ctx, gradx, gradgeom):
        """ Backward function for QuickCumsum Class """
...
class QuickCumsumCuda(torch.autograd.Function):
    """ QuickCumsumCuda Class """

    @staticmethod
    def forward(ctx, x, geom_feats, ranks, B, D, H, W):
        """ Forward function for QuickCumsumCuda Class """
        kept = torch.ones(x.shape[0], device=x.device, dtype=torch.bool)
        kept[1:] = ranks[1:] != ranks[:-1]
    ...
    def backward(ctx, out_grad):
        """ Backward function for QuickCumsumCuda Class """
