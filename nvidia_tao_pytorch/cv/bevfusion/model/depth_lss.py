...
"""BEVFusion View Transform modules"""
...
class BaseViewTransform(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            feature_size (Tuple[int, int]): The feature size.
            xbound (Tuple[float, float, float]): The grid range for x-axis
            ybound (Tuple[float, float, float]): The grid range for y-axis
            zbound (Tuple[float, float, float]): The grid range for z-axis
            dbound (Tuple[float, float, float]): The grid range for depth
        """
        super().__init__()
    ...
    def forward(
        ...
        **kwargs,
        ...
        """ Forward function for base view transform """
...
class LSSTransform(BaseViewTransform):
    ...
    def forward(self, *args, **kwargs):
        """ forward function for LSS Transform """
...
class BaseDepthTransform(BaseViewTransform):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """ forward function for BaseDepthTransform"""
        intrins = camera_intrinsics[..., :3, :3]
        post_rots = img_aug_matrix[..., :3, :3]
        post_trans = img_aug_matrix[..., :3, 3]
        camera2lidar_rots = camera2lidar[..., :3, :3]
...
class DepthLSSTransform(BaseDepthTransform):
    ...
    def forward(self, *args, **kwargs):
        """ forward function for DepthLSSTransform """
        x = super().forward(*args, **kwargs)
        x = self.downsample(x)
        return x
