...
class ConvFuser(nn.Sequential):
    ...
    def forward(self, inputs: List[torch.Tensor]) -> torch.Tensor:
        """ forward funciton for ConvFuser class"""
...
class BEVFusionHead(nn.Module):
    ...
    def __init__(
            ...
            bias='auto',
            # loss
            loss_cls={'type': 'mmdet.GaussianFocalLoss', 'reduction': 'mean'},
            loss_bbox={'type': 'mmdet.L1Loss', 'reduction': 'mean'},
            loss_heatmap={'type': 'mmdet.GaussianFocalLoss', 'reduction': 'mean'},
            # others
            bbox_coder=None,
            train_cfg=None,
            test_cfg=None):
        """
        Args:
            num_proposals (int): Number of proposal.
            auxiliary (bool): Whether to enable auxiliary training. Defaults to True.
            in_channels (int): Number of channels in the input feature map.
            hidden_channel (int): Number of hiden channel.
            num_classes (int): Number of classes.
            num_decoder_layers (int): Number of decoder layer.
            decoder_layer (Dict): decoder layer parameters
            num_heads (int): Number of heads.
            nms_kernel_size (int): NMS kernel size
            bn_momentum (float): Batch Norm momentum
            common_heads (Dict): Common Head parameters
            num_heatmap_convs: Number of heatmap convolutions
        ...
        for i in range(self.num_decoder_layers):
            ...
            heads.update({'heatmap': (self.num_classes, num_heatmap_convs)})
    ...
    def _init_assigner_sampler(self):
        ...
        if self.sampling:
            self.bbox_sampler = build_sampler(self.train_cfg.get("sampler", None))
        else:
            self.bbox_sampler = PseudoSampler()
        _assigner = self.train_cfg.get("assigner")
        if isinstance(_assigner, dict):
            self.bbox_assigner = build_assigner(_assigner)
        elif isinstance(_assigner, list):
            self.bbox_assigner = [
                build_assigner(res) for res in _assigner
            ]
    ...
    def forward_single(self, inputs, metas):
        ...
        elif self.test_cfg['dataset'] in ('TAO3DSyntheticDataset', 'TAO3DDataset', 'KittiPerson'):
            local_max = F.max_pool2d(heatmap, kernel_size=1, stride=1, padding=0)
    ...
    def predict(self, batch_feats, batch_input_metas):
        """ Predict function for BEVFusionHead """
        ...
        for _, preds_dict in enumerate(preds_dicts):
            ...
            for i in range(batch_size):
                ...
                if self.test_cfg['nms_type'] is not None:
                    ...
                    ret = {'bboxes': boxes3d, 'scores': scores, 'labels': labels}
            ...
            if 'vel' in preds_dict.keys():
                layer_vel = preds_dict['vel'][..., idx_layer *
                                              self.num_proposals:(idx_layer +
                                                                  1) *
                                              self.num_proposals, ]
                preds = torch.cat([layer_center, layer_height, layer_dim,
                                   layer_rot, layer_vel], dim=1).permute(0, 2, 1)  # [BS, num_proposals, code_size]
