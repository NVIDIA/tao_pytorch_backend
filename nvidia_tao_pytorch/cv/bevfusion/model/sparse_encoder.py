...
"""BEVFusion sparse encoder modules"""
...
class BEVFusionSparseEncoder(SparseEncoder):
    """Sparse encoder for BEVFusion. The difference between this
    ...
    def __init__(self,
        ...
        assert block_type in ['conv_module', 'basicblock'], 'block_type must be conv_module or basicblock'
        ...
        assert (isinstance(order, (list, tuple))) and len(order) == 3, \
            'the length of order must be 3 and type must be list or tuple'
        assert set(order) == {'conv', 'norm', 'act'}, 'order must be (conv, norm, act)'
