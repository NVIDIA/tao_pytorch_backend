...
"""BEVFusion evaluation functions """
...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    """ clean data functions to remove outliers"""
    CLASS_NAMES = ['person']
    MIN_HEIGHT = [40, 25, 25]
    MAX_OCCLUSION = [0, 1, 2]
    ...
    for i in range(num_gt):
        ...
        if (gt_name == current_cls_name):
            ...
            valid_class = -1
        ignore = False
        occ_check = gt_anno['occluded'][i] != 0 and gt_anno['occluded'][i] > MAX_OCCLUSION[difficulty]
        trunc_check = gt_anno['truncated'][i] != 0 and (gt_anno['truncated'][i] > MAX_TRUNCATION[difficulty])
        height_check = height != 0 and height <= MIN_HEIGHT[difficulty]
        if occ_check or trunc_check or height_check:
            ignore = True
    ...
    for i in range(num_dt):
        ...
        if (height != 0 and height < MIN_HEIGHT[difficulty]):
            ignored_dt.append(1)
        if valid_class == 1:
            ignored_dt.append(0)
        else:
...
def d3_box_overlap(boxes, qboxes, criterion=-1):
    """ box overlap kernel function"""
    from .rotate_iou import rotate_iou_gpu_eval
    rinc = rotate_iou_gpu_eval(boxes[:, [0, 2, 3, 5, 7]],
                               qboxes[:, [0, 2, 3, 5, 7]], 2)
...
def calculate_iou_partly(dt_annos, gt_annos, metric, num_parts=50):
    ...
    Args:
        dt_annos (dict): Must from get_label_annos() in tao3d_common.py.
        gt_annos (dict): Must from get_label_annos() in tao3d_common.py.
        metric (int): Eval type. 0: bbox, 1: bev, 2: 3d.
        num_parts (int): A parameter for fast calculate algorithm.
    """
    assert len(dt_annos) == len(gt_annos), 'GT and Prediction lengths must be same'
    ...
    for num_part in split_parts:
        ...
        if metric == 0:
            ...
            rots = np.concatenate([a['rotation'] for a in dt_annos_part], 0)
...
def eval_class(gt_annos,
    ...
    """
    assert len(gt_annos) == len(dt_annos), 'GT and Prediction lengths must be same'
...
def do_eval(gt_annos,
    ...
    """ perform evaluation on difficulty options """
...
def tao3d_eval(gt_annos,
    ...
    mAP11_bbox, mAP11_bev, mAP11_3d, mAP11_aos, mAP40_bbox, mAP40_bev, mAP40_3d, mAP40_aos = do_eval(gt_annos, dt_annos,
    ...
    if len(current_classes) > 1:
        ...
        for idx in range(len(difficulty)):
            ...
            if mAP11_3d is not None:
                ret_dict[f'TAO3D/Overall_3D_AP11_{postfix}'] = mAP11_3d[idx, 0]
            if mAP11_bev is not None:
                ret_dict[f'TAO3D/Overall_BEV_AP11_{postfix}'] =\
                    mAP11_bev[idx, 0]
            if mAP11_bbox is not None:
                ret_dict[f'TAO3D/Overall_2D_AP11_{postfix}'] =\
                    mAP11_bbox[idx, 0]
    ...
    if len(current_classes) > 1:
        ...
        for idx in range(len(difficulty)):
            postfix = f'{difficulty[idx]}'
            if mAP40_3d is not None:
                ret_dict[f'TAO3D/Overall_3D_AP40_{postfix}'] = mAP40_3d[idx, 0]
            if mAP40_bev is not None:
                ret_dict[f'TAO3D/Overall_BEV_AP40_{postfix}'] =\
                    mAP40_bev[idx, 0]
            if mAP40_bbox is not None:
                ret_dict[f'TAO3D/Overall_2D_AP40_{postfix}'] =\
                    mAP40_bbox[idx, 0]

    return result, ret_dict


def tao3d_eval_coco_style(gt_annos, dt_annos, current_classes):
    """coco style evaluation of tao3d.
