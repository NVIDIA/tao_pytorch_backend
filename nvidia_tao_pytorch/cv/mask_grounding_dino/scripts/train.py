...
def run_experiment(experiment_config):
    ...
    if experiment_config.train.activation_checkpoint and \
        ...
        logger.info("Disabling  activation checkpointing since model is smaller")
    ...
    if pretrained_path:
        ...
        checkpoint = load_pretrained_weights(pretrained_path, parser=mask_gdino_parser, ptm_adapter=ptm_adapter)
    ...
    if len(trainer_kwargs['devices']) > 1:
        ...
        if distributed_strategy.lower() == "ddp" and activation_checkpoint:
            ...
            logger.info("Overriding Precision to FP16 for fsdp")
            precision = '16-mixed'
        else:
...
@monitor_status(name="Mask Grounding DINO", mode="train")
