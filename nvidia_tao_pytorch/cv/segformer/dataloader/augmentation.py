...
def pil_crop(image, box, cropsize, default_value):
    ...
    if not isinstance(image, Image.Image):
        raise TypeError(f"Invalid data type for img: {type(image)}. Expected Image.Image")
...
def pil_rescale(img, scale, order):
    ...
    """
    assert isinstance(img, Image.Image), f"Invalid data type during augmentation for img: {type(img)}. Expected Image.Image"
